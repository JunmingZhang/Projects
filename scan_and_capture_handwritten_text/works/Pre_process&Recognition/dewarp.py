...
Reference:
Zucker, Matt, page_dewarp, (2016), GitHub repository, https://github.com/mzucker/page_dewarp
"""
...
def debug_show(name, step, text, display):
    """
    Print the middle stage information or image by given option.
...
def round_nearest_multiple(i, factor):
    """
    Round i to the nearest multiple of factor f
...
def pix2norm(shape, pts):
    """
    Calculate the norm of the pixel
...
def fltp(point):
    """
    Get the flattened tuple of a given point
...
def draw_correspondences(img, dstpoints, projpts):
    """
    Draw the correspondence at the image of the 
    real keypoints and projected keypoints in different
    color
...
def get_default_params(corners, ycoords, xcoords):
    """
    Get the default page information(parameters) such as
    width, height, initial entry of cubic slope, page rotation
    matrix
...
def project_xy(xy_coords, pvec):
    """
    Project the xy coordinates to the given cubic polynomial
...
def project_keypoints(pvec, keypoint_index):
    """
    Project the keypoints to the given cubic polynomial
...
def resize_to_screen(src, maxw=1280, maxh=700, copy=False):
    """
    Resize the screen to a given dimension
...
def box(width, height):
    """
    Get a white box with given dimension.
    The purpose is to use white box to fill out the gap of contours
...
def get_page_extents(small):
    """
    Get the cut out page information with the same dimension as the 
    given mask
...
def get_mask(name, small, pagemask, masktype):
    """
    Get the mask with given parameters
...
def interval_measure_overlap(int_a, int_b):
    """
    Determine whether the interval overlaps
...
def angle_dist(angle_b, angle_a):
    """
    Get the angle of difference.
    Standardized in domain: [-pi, pi]
...
def blob_mean_and_tangent(contour):
    """
    Calculate the mean of the tangent of the blob of the contour
...
class ContourInfo(object):
    ...
    def local_overlap(self, other):
        """
        Determine whether there is a local overlap
...
def generate_candidate_edge(cinfo_a, cinfo_b):
    """
    Generate the successive candidate edge by minimize the maximum overlap of x to a and b
...
def make_tight_mask(contour, xmin, ymin, width, height):
    """
    Tightening the mask by tightening the contour at the minimum position
...
def get_contours(name, small, pagemask, masktype):
    """
    Thereshold and get the contour of the given mask
...
def assemble_spans(name, small, pagemask, cinfo_list):
    """
    Generate spans first sorting the candidate edges and try to connect them
...
def keypoints_from_samples(name, small, pagemask, page_outline,
                           span_points):
    """
    Get the keypoints from selected samples and return the coordinates
...
def visualize_contours(name, small, cinfo_list):
    """
    Visualize the text contour in the image
...
def visualize_spans(name, small, pagemask, spans):
    """
    Visualize the spaned line of the cubic polinomial
...
def visualize_span_points(name, small, span_points, corners):
    """
    Visualize the spaned points calculated from cubic polynomial
...
def imgsize(img):
    """
    Get the printing format of image size
...
def make_keypoint_index(span_counts):
    """
    Get the keypoint index of the given span
...
def optimize_params(name, small, dstpoints, span_counts, params):
    """
    Solve the optimization problem with all the params and the objective function.
    By Minimizing the squared distance between calculated points using estimated 
    cubic span and the real points.
...
def get_page_dims(corners, rough_dims, params):
    """
    Get the optimized (cutted) dimension of the page that
    can contain the main information.
...
def remap_image(name, img, small, page_dims, params):
    """
    Once get the optimized parameters and specific page informations,
    map the transformed information(using params) to the page mask.
...
def main():
    ...
    for imgfile in sys.argv[1:]:
        ...
        print('loaded', basename, 'with size', imgsize(img), 'and resized to', imgsize(small))
        ...
        print('  got', len(spans), 'spans', 'with', sum([len(pts) for pts in span_points]), 'points.')
